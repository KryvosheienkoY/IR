	...
	void readEntry() {
		...
		for (int i = 0; i < folder.listFiles().length; i++) {
			try {
				...
				FictionBook fb = new FictionBook(new File(path));
				if (fb == null)
					break;

				// authors
				ArrayList<Person> authorList = fb.getAuthors();
				...
				// titles
				String[] title = fb.getTitle().split(" ");
				...
				for (int j = 0; j < sections.size(); j++) {
					for (int k = 0; k < sections.get(j).getElements().size(); k++) {
						if (sections.get(j).getElements().get(k).getText() != null) {
							String[] content = sections.get(j).getElements().get(k).getText().split(" ");
		...
		addToFile(authors, "authors.txt");
		addToFile(titles, "titles.txt");
		addToFile(body, "body.txt");
	}
